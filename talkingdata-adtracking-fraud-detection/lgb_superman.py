...
def lgb_modelfit_nocv(params, dtrain, dvalid, predictors, target='target', objective='binary', metrics='auc',
    ...
    lgb_params = {
        ...
        'reg_alpha': 0.75,  # L1 regularization term on weights
...
def DO(skip_rows,random_seed):
    ...
    if debug:
        train_df = pd.read_csv("../input/train.csv", parse_dates=['click_time'], nrows=100000, dtype=dtypes, usecols=['ip','app','device','os', 'channel', 'click_time', 'is_attributed'])
    else:
        train_df = pd.read_csv("../input/train.csv", parse_dates=['click_time'], skiprows=skip_rows, dtype=dtypes, usecols=['ip','app','device','os', 'channel', 'click_time', 'is_attributed'])

    print('loading test data...')
    if debug:
        test_df = pd.read_csv("../input/test_supplement.csv", skiprows=range(1,21290880), nrows=100000, parse_dates=['click_time'], dtype=dtypes, usecols=['ip','app','device','os', 'channel', 'click_time', 'click_id'])
        id_map = pd.read_csv("../input/mapping.csv", dtype={'click_id':'uint32','old_click_id':'uint32'})
    else:
    ...
    if debug:
        val_idx = np.random.choice(np.arange(len_train), size=len_train//2, replace=False)
        val_df = train_df.iloc[val_idx]      
    else:
        val_idx = np.random.choice(np.arange(len_train), size=len_train//20, replace=False)
        val_df = train_df.iloc[val_idx]
    ...
    sub = pd.DataFrame()
    sub['old_click_id'] = test_df['click_id'].astype('int')
