...
def transpose_image(X_img):
    if len(X_img.shape)==3:
        return np.transpose(X_img,(1,0,2))
    elif len(X_img.shape)==2:
        return np.transpose(X_img)
...
def load_image_gt(dataset, config, image_id, augment=False,
    ...
    if augment:
        ...
        if random.random()>0.8:
            rand_proportion = random.randint(5,9)
            rand_proportion = rand_proportion*0.1
            start_proportion = 1-rand_proportion

            img_shape = image.shape
            msk_shape = mask.shape
            startx = random.randint(0,int(start_proportion*img_shape[0]))
            starty = random.randint(0,int(start_proportion*img_shape[1]))
            if len(img_shape)==3:
                image = image[startx:startx+int(rand_proportion*img_shape[0]),
                              starty:starty+int(rand_proportion*img_shape[1]),:]
            else:
                image = image[startx:startx+int(rand_proportion*img_shape[0]),
                              starty:starty+int(rand_proportion*img_shape[1])]
            if len(msk_shape)==3:
                mask = mask[startx:startx+int(rand_proportion*img_shape[0]),
                              starty:starty+int(rand_proportion*img_shape[1]),:]
            else:
                mask = mask[startx:startx+int(rand_proportion*img_shape[0]),
                              starty:starty+int(rand_proportion*img_shape[1])]
...
class MaskRCNN():
    ...
    def train(self, train_dataset, val_dataset, learning_rate, epochs, layers):
        ...
        callbacks = [
            ...
            keras.callbacks.EarlyStopping(monitor='val_mrcnn_mask_loss', patience=20, verbose=0, mode='auto')
        ]
